...
export const INDUSTRIES = {
  ...
  indoor_environmental: {
    ...
    agents: [
      { name: "Inquiry Response Agent", icon: "PhoneIncoming", dept: "Intake", description: "Answers every mold, air quality, and EMF inquiry 24/7 and starts the booking flow", status: "active", tasks: ["Responds to website forms and calls within 60–90 seconds", "Triages service type: mold vs air quality vs EMF vs commercial", "Collects property details and health symptom context", "Hands warm leads directly into booking workflow"], kpis: { primary: "Inquiries Captured", secondary: "Booking Rate" }, metric: { value: 88, unit: "%", label: "Capture Rate" } },
      { name: "Assessment Scheduler", icon: "CalendarCheck", dept: "Intake", description: "Books inspections automatically and removes phone-tag delays", status: "active", tasks: ["Books assessments based on inspector availability and zone", "Groups same-area inspections to reduce drive time", "Sends 24hr and 2hr confirmation reminders", "Handles reschedules without the owner's involvement"], kpis: { primary: "Appointments Booked", secondary: "Show Rate" }, metric: { value: 90, unit: "%", label: "Show Rate" } },
      { name: "Lead Follow-Up Agent", icon: "RefreshCw", dept: "Intake", description: "Reactivates people who asked questions but never booked", status: "active", tasks: ["Follows up with unconverted leads by text and email", "Revives estimates and inspection inquiries that went cold", "Prompts quick booking with clear next steps", "Routes hot replies back to scheduling automatically"], kpis: { primary: "Lead Revival", secondary: "Recovered Jobs" }, metric: { value: 14, unit: "%", label: "Revival Rate" } },
      { name: "Payment Recovery Agent", icon: "BadgeDollarSign", dept: "Finance", description: "Chases outstanding invoices professionally and speeds up collections", status: "active", tasks: ["Sends invoice reminders by SMS and email", "Handles payment link follow-ups automatically", "Escalates overdue accounts with context", "Reduces time-to-payment without staff chasing manually"], kpis: { primary: "Collections Recovered", secondary: "Days to Pay" }, metric: { value: 54, unit: "%", label: "Recovery Rate" } },
      { name: "Client Update Agent", icon: "MessageSquareMore", dept: "Client Success", description: "Keeps clients updated at every stage so they stop chasing the office", status: "active", tasks: ["Confirms bookings and sends prep reminders", "Updates clients when inspector is en route", "Confirms report delivery and next steps", "Sends remediation and retest status updates automatically"], kpis: { primary: "Status Calls Reduced", secondary: "Client Satisfaction" }, metric: { value: 58, unit: "%", label: "Calls Reduced" } },
      { name: "Review & Reputation Agent", icon: "Star", dept: "Growth", description: "Captures 5-star reviews after the core follow-up workflows are already in place", status: "active", tasks: ["Sends personalized review request 24hr after report delivery", "One-tap Google review link — zero friction", "2-touch follow-up sequence for non-responders", "Monitors and responds to Google/Yelp reviews professionally"], kpis: { primary: "Review Conversion", secondary: "Avg Rating" }, metric: { value: 24, unit: "%", label: "Review Conversion" } },
      { name: "Environmental Education Agent", icon: "FileText", dept: "Client Success", description: "Builds trust and authority through targeted pre- and post-visit education", status: "active", tasks: ["Sends service-specific education before every inspection", "Explains mold, VOC, radon, and EMF health effects in plain language", "Demystifies report findings before the follow-up call", "Positions IET as trusted expert vs faceless competitor"], kpis: { primary: "Emails Opened", secondary: "Upsell Rate" }, metric: { value: 52, unit: "%", label: "Open Rate" } }
    ],

    workflow: [
      { step: "Inquiry", system: "Inquiry Response Agent", icon: "PhoneIncoming", before: "Goes to voicemail after hours or during field work", after: "Answered in about 90 seconds and qualified automatically", beforeTime: "Next day", afterTime: "90 seconds", dropoff: { before: 30, after: 6 } },
      ...
      { step: "Lead Revival", system: "Lead Follow-Up Agent", icon: "RefreshCw", before: "Unbooked enquiries disappear forever", after: "Cold leads reactivated automatically within 24 hours", beforeTime: "Never", afterTime: "Same day", dropoff: { before: 18, after: 8 } },
      ...
      { step: "Payments", system: "Payment Recovery Agent", icon: "BadgeDollarSign", before: "Invoices chased manually or forgotten", after: "Payment reminders and follow-up sent automatically", beforeTime: "Delayed", afterTime: "Same day", dropoff: { before: 12, after: 4 } },
      { step: "Reviews", system: "Review & Reputation Agent", icon: "Star", before: "Review asked by chance — maybe 1 in 10 clients posts", after: "Systematic ask 24hr post-report — 24% conversion rate", beforeTime: "Rarely", afterTime: "Every job", dropoff: { before: 0, after: 0 } }
    ],

    roi: { leadResponseImprovement: 0.45, conversionLift: 0.11, missedLeadRecovery: 0.18, avgResponseTimeBefore: "Next day", avgResponseTimeAfter: "90 seconds", reviewIncreasePercent: 150 },
    ...
    painPoints: [
      ...
      { title: "After-Hours Inquiries Lost", stat: "20%+", description: "of web inquiries arrive when owner is unavailable — first-responder wins the booking", icon: "PhoneOff" },
      { title: "Unconverted Leads Never Revisited", stat: "10%+", description: "warm prospects who asked questions disappear because no automated follow-up exists", icon: "RefreshCw" },
      { title: "Outstanding Payments Need Chasing", stat: "40%+", description: "of overdue invoices could be recovered faster with systematic reminders and payment links", icon: "BadgeDollarSign" }
    ],
    ...
    dayStory: {
      before: [
        ...
        { time: "NEXT MORNING", event: "Competitor who had live chat responded at 8:52 PM", detail: "Booked the appointment. $385 inspection gone before breakfast.", icon: "❌", status: "lost" },
        ...
        { time: "RESULT", event: "$385 inspection lost overnight", detail: "Because a worried parent couldn't wait until morning.", icon: "💸", status: "lost" }
      ],
      after: [
        { time: "8:45 PM", event: "Parent Googles 'mold inspector Nashville'", detail: "Saw black spots near the baby's room — scared, ready to book tonight", icon: "🔍" },
        { time: "8:46 PM", event: "Submits contact form — AI responds in about 90 seconds", detail: "Empathetic response: 'We understand how concerning this is. Let's get you scheduled.'", icon: "⚡", status: "active" },
